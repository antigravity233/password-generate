...
const generatePassword = () => {
    ...
    options.forEach(option => { 
        ...
        if(option.checked){ //if checkbox is checked
            ...
            } else {// else pass true value to excludeDeplicate
    ...
    for(let i = 0; i < passLength; i++) {
        // getting random character form the ststic password
        ...
        if (excludeDuplicate) { //if excludeDuplicate is true
            ...
            // to space " " then add random chracter to randomPassword else decrement i by -1
    ...
    passwordInput.value = randomPassword; // passing randomPasseord to passwordInnout value
}

const updatePassIndicator = () => {
    // if lengthSlider value is less than 8 the pass "weak" as passIndicator id else if lengthSlider
    // value is less than 16 then pasd "medium" as id else pass "strong" as id
...
const copyPassword = () => {
    ...
    setTimeout(() => { // after 1500ms, changing tick back to copy_all
